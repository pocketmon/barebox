...
// <<390 PKM support kernel XIP
	//printf("os_addr %08X header load addr:%08X  entery addr:%08X\n",
		//	data->os_address, data->os->header.ih_load, data->os->header.ih_ep);
	if (data->os_address != data->os->header.ih_load)
	{
	...
	{
		data->os_res = xipimage_sdram_setup(data->os, data->os_address, data->os->header.ih_size);
		data->os_entry = 0x40;
	}
// 390 PKM>>
