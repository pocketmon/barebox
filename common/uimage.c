...
// <<390 PKM supprt kernel XIP
/*
 * Load an uImage to a dynamically allocated sdram resource.
 * the resource must be freed afterwards with release_sdram_region
 */
struct resource *xipimage_sdram_setup(struct uimage_handle *handle,
		unsigned long start, unsigned long size)
{
	uimage_resource = xzalloc(sizeof(*uimage_resource));
	uimage_resource->name = "uimage";
	uimage_resource->start = start;
	uimage_resource->end = start + size - 1;

	return uimage_resource;
}
EXPORT_SYMBOL(xipimage_sdram_setup);
// 390 PKM>>
